...
Bộ năng lực dùng chung giữa **Codex (local)** và **ChatGPT Work**, xuất từ MCP `codexx_agent_skills` (`/Users/davidtran/agent-skills`).

- **24 skills** — workflow 'làm thế nào' (mỗi skill 1 `SKILL.md`, kèm scripts/refs nếu có).
- **4 personas** — vai trò review (mỗi persona 1 `SKILL.md`).
- **10 rules** — quy ước, vòng đời, checklist.
- **orchestrator/SKILL.md** — điều phối bắt buộc: map intent→skill + áp vòng đời.
- **commands/** — slash command tham khảo (build/plan/review/ship/spec/test/webperf).

## Cách ChatGPT Work dùng
1. Đọc `orchestrator/SKILL.md` trước — đây là điểm vào.
2. Với mỗi yêu cầu, map intent → skill trong `skills/`, làm theo đúng workflow.
3. Nạp `rules/` làm Project Instructions (đặc biệt `agents.md`, `definition-of-done.md`, `orchestration-patterns.md`).
4. Đóng vai persona trong `personas/` khi review (5 trục: correctness, readability, architecture, security, performance).
...
- `rules/observability-checklist.md`
- `rules/orchestration-patterns.md`
- `rules/performance-checklist.md`
- `rules/readme.md`
- `rules/security-checklist.md`
- `rules/testing-patterns.md`

> Nguồn gốc, đầy đủ. Không chứa secret/token/.env. Xem `MANIFEST.json` để có danh sách file chính xác.